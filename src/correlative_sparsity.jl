function Pu_sparse_hierarchy(x,f0,g,h,I,J,W,k)
    ...
    u=Vector{Vector{Int16}}(undef, p)
    w=Vector{Vector{Int16}}(undef, p)

    lu0=Vector{UInt16}(undef, p)
    lu=Vector{Vector{UInt16}}(undef, p)
    lw=Vector{Vector{UInt16}}(undef, p)
